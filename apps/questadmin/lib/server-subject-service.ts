/**
 * Server-side Subject Service
 * Handles subject operations on the server with proper Firebase access
 */
...
export async function getSubjectsByProgram(programId: string): Promise<Subject[]> {
  try {
    ...
    const q = query(
      collection(serverDb, SUBJECTS_COLLECTION),
...
export async function getSubjectById(subjectId: string): Promise<Subject | null> {
  try {
    const docRef = doc(serverDb, SUBJECTS_COLLECTION, subjectId)
...
export async function createSubject(
  ...
  try {
    ...
    const docRef = await addDoc(collection(serverDb, SUBJECTS_COLLECTION), subjectData)
    return docRef.id
  } catch (error) {
...
export async function updateSubject(request: UpdateSubjectRequest, updatedBy: string): Promise<void> {
  try {
    ...
    const docRef = doc(serverDb, SUBJECTS_COLLECTION, id)
...
export async function getAvailableInstructors(collegeId: string): Promise<InstructorOption[]> {
  try {
    const q = query(
      collection(serverDb, USERS_COLLECTION),
...
export async function getInstructorName(instructorId: string): Promise<string> {
  try {
    const docRef = doc(serverDb, USERS_COLLECTION, instructorId)
    const docSnap = await getDoc(docRef)
