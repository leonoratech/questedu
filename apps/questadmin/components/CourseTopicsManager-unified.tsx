...
export function CourseTopicsManager({ courseId, isEditable, multilingualMode = false }: CourseTopicsManagerProps) {
  ...
  const loadTopics = async () => {
    try {
      ...
      setTopics(hybridTopics.sort((a, b) => a.order - b.order))
    } catch (err) {
    ...
    try {
      const topicData: CreateCourseTopicData = {
        ...
        learningObjectives: multilingualMode
          ? getCompatibleArray(formData.learningObjectives as MultilingualArray, DEFAULT_LANGUAGE).filter(obj => obj.trim())
  ...
  const startEditing = (topic: HybridTopic) => {
    setFormData({
      title: multilingualMode 
        ? (typeof topic.title === 'string' ? createMultilingualText(topic.title) : topic.title)
        : (typeof topic.title === 'string' ? topic.title : getCompatibleText(topic.title, DEFAULT_LANGUAGE)),
      description: multilingualMode
        ? (typeof topic.description === 'string' ? createMultilingualText(topic.description || '') : topic.description || createMultilingualText(''))
      ...
      learningObjectives: multilingualMode
        ? (Array.isArray(topic.learningObjectives) ? createMultilingualArray(topic.learningObjectives) : topic.learningObjectives || createMultilingualArray([]))
    ...
    <Card className="mb-6">
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          {multilingualMode ? (
            <MultilingualArrayInput
              ...
              value={formData.learningObjectives as MultilingualArray}
        ...
        <div className="flex gap-2 pt-4 border-t">
          <Button onClick={editingTopic ? () => handleUpdateTopic(editingTopic) : handleAddTopic}>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold flex items-center gap-2">
          {multilingualMode && <Globe className="h-6 w-6 text-blue-600" />}
          {multilingualMode ? 'Multilingual Course Topics' : 'Course Topics'}
        </h2>
                ...
                {topic.description && (
                  <CardContent className="pt-0">
                    ...
                    {topic.learningObjectives && (
                      multilingualMode 
                        ? getCompatibleArray(topic.learningObjectives as MultilingualArray, DEFAULT_LANGUAGE).length > 0
                        : (topic.learningObjectives as string[]).length > 0
                    ) && (
                      <div className="mb-4">
                        <h4 className="font-medium mb-2">Learning Objectives</h4>
                        <ul className="list-disc list-inside space-y-1">
                          {(multilingualMode 
                            ? getCompatibleArray(topic.learningObjectives as MultilingualArray, DEFAULT_LANGUAGE)
        ...
        {topics.length === 0 && !showAddForm && (
          <Card>
            <CardContent className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                Add {multilingualMode ? 'multilingual ' : ''}topics to structure your course content and help students learn step by step.
              </p>
              {isEditable && (
                <Button onClick={() => setShowAddForm(true)}>
                  <Plus className="h-4 w-4 mr-2" />
